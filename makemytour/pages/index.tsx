...
export default function Home() {
  ...
  const cityOptions = useMemo(() => {
    ...
    console.log("Flight state:", flight);
console.log("Hotel state:", hotel);
  ...
  //       (flight) =>
  //         flight.from.toLowerCase().trim() === from.toLowerCase().trim() &&
  //         flight.to.toLowerCase().trim() === to.toLowerCase().trim()
  //     );
  ...
  const results = flight.filter((item: any) => {
    return (
      item.from.toLowerCase().trim() === from.toLowerCase().trim() &&
      item.to.toLowerCase().trim() === to.toLowerCase().trim()
    );
  });
  ...
  const handlebooknow = (_id: any) => {
    console.log("Booking ID:", _id);

    if (bookingtype === "flights") {
      router.push(`/book-flight/${_id}`);
    } else {
      router.push(`/book-hotel/${_id}`);
    }
  };
  return (
    <div
