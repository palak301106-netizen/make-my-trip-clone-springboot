...
import { getDynamicPrice, getflight, handleflightbooking, getFlightStatus } from "@/api";
...
const BookFlightPage = () => {
    ...
    const fetchFlights = async () => {
      try {
        ...
        const filteredData = data.filter((flight: any) => {
          console.log("Comparing:", flight.id, id);
          return flight.id === id;
        });
  ...
  useEffect(() => {
    ...
    const loadFlightStatus = async () => {
      ...
      const flightId = flights[0]._id || flights[0].id;
  ...
  //     const isWeekend =
  //       new Date(flight.departureTime).getDay() === 0 ||
  //       new Date(flight.departureTime).getDay() === 6;

  //     const data = await getDynamicPrice(
  //       flight.price,
  //       isWeekend,
  //       false
  //     );
    ...
    <div className="grid grid-cols-4 gap-4 max-w-sm">
      {seats.map((seat) => (
        <button
          ...
          className={`p-3 rounded-lg border font-semibold transition ${selectedSeat === seat
    ...
    <DialogContent className="sm:max-w-[600px] bg-white">
      ...
      <div className="grid gap-6 mt-4">
        ...
        <div className="space-y-2">
          ...
          <Input
            value={selectedSeat || "No seat selected"}
            readOnly
          />
        </div>
            ...
            <div className="mt-6 bg-blue-50 rounded-lg p-4">
              ...
              <p>
                ...
                <span className="font-semibold text-green-600">
                  {" "}
                  {flightStatus?.status || "Loading..."}
                </span>
              </p>

              <p>
                Delay Reason :
                {flightStatus?.delayReason || "No Delay"}
              </p>
              ...
              <div className="grid grid-cols-4 gap-2 max-w-[220px]">
                {seats.map((seat) => (
                  <button
                    key={seat}
                    onClick={() => setSelectedSeat(seat)}
                    className={`h-10 w-10 rounded-md border text-sm font-medium transition ${selectedSeat === seat
                      ? "bg-blue-600 text-white border-blue-600"
            ...
            {/* <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span>
                    {flightStatus?.delayReason || "Loading..."}
                  </span>
                </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
              {hotels.map((hotel, index) => (
                <div
                  key={index}
                  className="w-full bg-white border rounded-xl overflow-hidden hover:shadow-md transition-shadow"
                >
